...
# ==========================================
# BINNING HELPERS FOR PRIVATE PGM
# ==========================================

def bin_dataframe(df, label_col, n_bins=4):
    """
    Bin continuous features into n_bins integer categories (0 to n_bins-1)
    using quantile-based binning. Returns binned DataFrame and per-feature
    bin means (for later reconstruction to continuous).
    """
    binned_df = df.copy()
    bin_means = {}
    ...
    for col in df.columns:
        if col == label_col:
            continue
        try:
            binned, _ = pd.qcut(df[col], q=n_bins, labels=False, retbins=True, duplicates='drop')
        except Exception:
            binned = None

        if binned is None or binned.isna().all():
            # Constant column: assign bin 0
            binned_df[col] = 0
            bin_means[col] = [float(df[col].mean())] * n_bins
            continue

        # Fill NaN edges (can happen with qcut on low-variance cols) with 0
        binned = binned.fillna(0).astype(int)

        # Ensure bins are in [0, n_bins-1]
        binned = binned.clip(0, n_bins - 1)

        # Compute mean of the original continuous values within each bin
        means = []
        for b in range(n_bins):
            vals = df.loc[binned == b, col]
            means.append(float(vals.mean()) if len(vals) > 0 else 0.0)
        bin_means[col] = means
        binned_df[col] = binned

    return binned_df, bin_means


def bins_to_continuous(synth_array, feature_cols, label_col, bin_means):
    """
    Convert a synthetic array (bin indices + label) back to continuous values
    using the training-set bin means.

    synth_array: numpy array with columns [features..., label] in feature_cols order + label last
    Returns a DataFrame with the original column names.
    """
    all_cols = list(feature_cols) + [label_col]
    continuous_df = pd.DataFrame(synth_array, columns=all_cols)

    for col in feature_cols:
        if col not in bin_means:
            continue
        means = bin_means[col]
        def map_bin(b):
            idx = int(b)
            if 0 <= idx < len(means):
                return means[idx]
            return 0.0
        continuous_df[col] = continuous_df[col].apply(map_bin)
...
def run_benchmark(full_data_path, label_column, feature_sizes, epsilon=10.0,
    ...
    results = []
    raw_log_file = f"{prefix}benchmark_pgm_fidelity_RAW_log.csv"

    for n_features in feature_sizes:
        print(f"\n{'='*80}")
        print(f" BENCHMARK (Private PGM): {n_features} FEATURES | eps={epsilon} | RUNS: {n_runs}")
        print(f"{'='*80}")
        ...
        for run_id in range(n_runs):
            print(f"\n--- [Features: {n_features_to_use}] Run {run_id + 1}/{n_runs} ---")
            ...
            try:
                # ---- Bin training data for Private PGM ----
                binned_train_df, bin_means = bin_dataframe(train_df, 'label', n_bins=n_bins)

                num_classes = len(train_df['label'].unique())
                feature_cols = [c for c in binned_train_df.columns if c != 'label']
                config = {col: n_bins for col in feature_cols}
                config['label'] = num_classes

                # ---- Train Private PGM (no MPC) ----
                pgm = Private_PGM(
                ...
                # ---- Convert bins back to continuous ----
                synth_df = bins_to_continuous(synth_array, feature_cols, 'label', bin_means)
        ...
        print(f"\n--- Averaging metrics across {len(run_metrics_list)} successful runs ({n_features_to_use} features) ---")
        avg_metrics = {
    ...
    print("\n" + "="*80)
    print(f" FINAL BENCHMARK RESULTS (Private PGM)")
    print("="*80)
    ...
    out_filename = f"{prefix}benchmark_pgm_fidelity_avg.csv"
    results_df.to_csv(out_filename, index=False)
    print(f"✓ Averaged results saved to: {out_filename}")
    print(f"✓ Raw un-averaged results safely preserved in: {raw_log_file}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Benchmark Private-PGM (no MPC) across feature sizes (gene scaling)"
    )
    parser.add_argument('--data', type=str, required=True, help='Path to dataset CSV')
    parser.add_argument('--label', type=str, required=True, help='Name of label column')
    parser.add_argument('--epsilon', type=float, default=10.0, help='DP epsilon (default: 10.0)')
