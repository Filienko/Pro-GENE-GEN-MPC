...
def run_benchmark(full_data_path, label_column, mpspdz_path, protocols, epsilons, n_runs=1):
    ...
    for current_protocol in protocols:
        for current_eps in epsilons:
            print(f"\n{'='*80}")
            print(f" BENCHMARK: PROTOCOL: {current_protocol} | EPSILON: {current_eps} | FEATURES: ALL ({n_total_features}) | RUNS: {n_runs}")
            print(f"{'='*80}")
            ...
            print(f"\n--- Averaging metrics across {len(run_metrics_list)} successful runs for {current_protocol} (Eps: {current_eps}) ---")
            avg_metrics = {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Wrapper to benchmark Secure MPC Pipeline + Biological Fidelity across Epsilons")
