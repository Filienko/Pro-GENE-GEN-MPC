...
# (identical to run_benchmark_epsilon.py)
# ==========================================
...
def bin_dataframe(df, label_col, n_bins=4):
    ...
    for col in df.columns:
        ...
        binned = binned.fillna(0).astype(int).clip(0, n_bins - 1)
...
def run_benchmark(full_data_path, label_column, epsilons, delta=1e-5,
                  n_runs=1, num_iters=10000, n_bins=4, prefix=""):
    ...
    results = []
    raw_log_file = f"{prefix}benchmark_pgm_epsilon_RAW_log.csv"

    for current_eps in epsilons:
        print(f"\n{'='*80}")
        print(f" BENCHMARK (Private PGM): EPSILON: {current_eps} | FEATURES: ALL ({n_total_features}) | RUNS: {n_runs}")
        print(f"{'='*80}")
        ...
        for run_id in range(n_runs):
            print(f"\n--- [Epsilon: {current_eps}] Run {run_id + 1}/{n_runs} ---")
            ...
            try:
                ...
                import traceback
                print(f"Pipeline failed for Epsilon {current_eps} on Run {run_id + 1}: {e}")
                traceback.print_exc()
                continue
        ...
        print(f"\n--- Averaging metrics across {len(run_metrics_list)} successful runs (Eps: {current_eps}) ---")
        avg_metrics = {
            'epsilon': current_eps,
    ...
    print("\n" + "="*80)
    print(f" FINAL BENCHMARK RESULTS (Private PGM, Epsilon Scaling)")
    print("="*80)
    ...
    cols = [
        'epsilon', 'num_features', 'base_accuracy', 'accuracy', 'base_f1', 'f1_score',
        'hist_intersection', 'dcr_knn', 'de_tpr', 'coex_tpr',
        'wasserstein_dist', 'feat_rank_tau', 'feat_topk_overlap', 'mare_zero_rate', 'mare_nz_mean',
        'corr_diff_mae', 'ari_real', 'ari_synth',
    ]
    ...
    out_filename = f"{prefix}benchmark_pgm_epsilon_avg.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Benchmark Private-PGM (no MPC) across epsilon values"
    )
