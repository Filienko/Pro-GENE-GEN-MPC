...
def run_benchmark(full_data_path, label_column, mpspdz_path, feature_sizes=[5, 10, 50, 100, 200, 500, 750, 1000], k=3):
    ...
    for n_features in feature_sizes:
        ...
        print(f"\n--- Starting MPC Pipeline ({k} runs, same data) ---")

        for i in range(k):

            print(f"\nRun {i+1}/{k}")

            # Reset MPC metrics per run
            for key in mpc_helper.MPC_METRICS:
                mpc_helper.MPC_METRICS[key] = 0 if isinstance(
                    mpc_helper.MPC_METRICS[key], int) else 0.0
            ...
            print(f"--- Evaluating Downstream Utility ---")
