...
# Adjusted feature_sizes: First run is 10k, the other 4 remain from the higher end of your original list
def run_benchmark(full_data_path, label_column, mpspdz_path, feature_sizes=[100, 200, 500, 750, 1000]):
    ...
    for n_features in feature_sizes:
        ...
        try:
            # Added mpc_script parameter to target the new optimized pipeline
            synth_df = run_secure_mpc_pipeline(
                ...
                marginal_protocol="histogram_marginals" # <--- Ensures we run the fast version
            )
